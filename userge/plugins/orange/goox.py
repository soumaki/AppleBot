...
@userge.on_cmd(
    "gx",
    about={
        "titulo": "Módulo que gera um adesivo animado de pesquisa do Google",
...
async def tweetando(message: Message):
    """Pesquisa de burro no Google - @applled"""
    ...
    if not args and len(quote_list) == 0:
        ...
        return
    try:
        async with userge.conversation("googlaxbot ", timeout=100) as conv:
            try:
                if quote_list:
                    await userge.forward_messages(
                        "googlaxbot ", message.chat.id, quote_list
                    )
                    if self_mid:
                        await message.delete()
                elif args:
                ...
                await message.edit("Dê /start no @googlaxbot ")
                return
