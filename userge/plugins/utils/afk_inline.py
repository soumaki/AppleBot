...
async def send_inline_afk(message: Message):
    ...
    await userge.send_inline_bot_result(
        chat_id=message.chat.id, query_id=x.query_id, result_id=x.results[0].id, hide_via=True,
    )
...
async def send_inline_afk_(message: Message):
    ...
    await userge.send_inline_bot_result(
        chat_id=message.chat.id, query_id=x_.query_id, result_id=x_.results[0].id, hide_via=True,
    )
...
async def _send_inline_afk(message: Message):
    ...
    await userge.send_inline_bot_result(
        chat_id=message.chat.id, query_id=_x.query_id, result_id=_x.results[0].id, hide_via=True,
    )
